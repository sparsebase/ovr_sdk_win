...
OculusWorldDemoApp::OculusWorldDemoApp() :
    ...
    DrawControllerLinearVelocity(false),
    DrawControllerAngularVelocity(false),
    DrawControllerLinearAccel(false),
    DrawControllerAngularAccel(false),
    ...
    LayerCubemapRenderResolution(1024),
...
int OculusWorldDemoApp::InitializeRendering(bool firstTime)
{
    ...
    HmdColorDesc = ovr_GetHmdColorDesc(Session);
    // Unknown is the default which gives a more appealing result
    HmdColorDesc.ColorSpace = ovrColorSpace_Unknown;
...
void OculusWorldDemoApp::PopulateOptionMenu()
{
    ...
    */
    ...
    Menu.AddEnum("Layers.Cubemap.Draw Type", &LayerCubemap).
        AddEnumValue("Disabled", Cubemap_Off).
        AddEnumValue("Rendered", Cubemap_Render).
        AddEnumValue("Texture", Cubemap_Load);
    Menu.AddBool("Layers.Cubemap.Save on Render", &LayerCubemapSaveOnRender);
    Menu.AddInt("Layers.Cubemap.Render Res Per-face", &LayerCubemapRenderResolution, 256, 8096, 256);
    Menu.AddBool("Layers.Cubemap.High Quality", &LayerCubemapHighQuality);
    ...
    Menu.AddBool("Player.Controller Vectors.Draw Linear Velocity", &DrawControllerLinearVelocity);
    Menu.AddBool("Player.Controller Vectors.Draw Angular Velocity", &DrawControllerAngularVelocity);
    Menu.AddBool("Player.Controller Vectors.Draw Linear Acceleration", &DrawControllerLinearAccel);
    Menu.AddBool("Player.Controller Vectors.Draw Angular Acceleration", &DrawControllerAngularAccel);
    ...
    OVR_STATIC_ASSERT(ovrColorSpace_Count == 9, "Update enum below");
    Menu.AddEnum("Display.HMD Color Space", &HmdColorDesc.ColorSpace).
      AddEnumValue("Unknown",   ovrColorSpace_Unknown).
      AddEnumValue("Unmanaged", ovrColorSpace_Unmanaged).
      AddEnumValue("Rift CV1",  ovrColorSpace_Rift_CV1).
      AddEnumValue("Rift S",    ovrColorSpace_Rift_S).
      AddEnumValue("Quest",     ovrColorSpace_Quest).
      AddEnumValue("Rec.2020",  ovrColorSpace_Rec_2020).
      AddEnumValue("Rec.709",   ovrColorSpace_Rec_709).
      AddEnumValue("P3",        ovrColorSpace_P3).
      AddEnumValue("AdobeRGB",  ovrColorSpace_Adobe_RGB).
      SetNotify(this, &OWD::HmdColorDescChange);
    ...
    Menu.AddEnum("SDK HUD.Perf HUD Mode 'F11'", &PerfHudMode).  AddEnumValue("Off", ovrPerfHud_Off).
                                                                ...
                                                                AddEnumValue("Oculus Link", ovrPerfHud_LinkPerf).
    ...
    else
    {
        for (int layerNum = 0; layerNum < CamLayerCount; layerNum++)
        {
            ...
            for (int eye = 0; eye < 2; eye++)
            {
                ...
                EyeLayer[layerNum].EyeFov.RenderPose[eye]     = EnableTimewarpOnMainLayer ? (OVR::Posef)eyeTrackingPose : (OVR::Posef)HmdToEyePose[eye];
...
void OculusWorldDemoApp::RenderEyeView(CamRenderPoseEnum camNum, ovrEyeType eyeNum, const Matrix4f* optionalMatrix, bool onlyRenderWorld)
{
    ...
    if ( (GridDisplayMode != GridDisplay_GridOnly) && (GridDisplayMode != GridDisplay_GridDirect) )
    {
        if (SceneMode == Scene_World || SceneMode == Scene_Cubes)
        {
            ...
            // Draws helpers showing the linear and angular velocities on each controller
            if (DrawControllerLinearVelocity || DrawControllerAngularVelocity ||
                DrawControllerLinearAccel || DrawControllerAngularAccel)
            {
              ovrTrackingState tracking = ovr_GetTrackingState(Session, HmdFrameTiming, ovrFalse);
              static int numVelocityCubes = 30;
              static float scaleStart = 5.0f;
              static float scaleEnd = 1.0f;
              const int numControllerPoses = OVR_ARRAY_COUNT(tracking.HandPoses);

              enum controllerVecType
              {
                ControllerVecType_LinVeloc,
                ControllerVecType_AngVeloc,
                ControllerVecType_LinAccel,
                ControllerVecType_AngAccel,
              };
              auto drawControllerVec = [&](Scene& cubeModel, float vecScale, controllerVecType vecType, bool inImuSpace)
              {
                for (int ctrl = 0; ctrl < numControllerPoses; ++ctrl)
                {
                  Posef worldPose = tracking.HandPoses[ctrl].ThePose;
                  // Angular velocity & angular acceleration are provided in IMU (controller) space,
                  // so we need to rotate using the controller poses.
                  if (inImuSpace) {
                    worldPose.Rotation = Quatf::Identity();
                  }
                  worldPose = ThePlayer.VirtualWorldTransformfromRealPose(worldPose, TrackingOriginType);

                  Matrix4f ctrlMat(worldPose);

                  Vector3f arrowVec = Vector3f::Zero();
                  switch (vecType)
                  {
                  case ControllerVecType_LinVeloc: arrowVec = (Vector3f)(tracking.HandPoses[ctrl].LinearVelocity); break;
                  case ControllerVecType_AngVeloc: arrowVec = (Vector3f)(tracking.HandPoses[ctrl].AngularVelocity); break;
                  case ControllerVecType_LinAccel: arrowVec = (Vector3f)(tracking.HandPoses[ctrl].LinearAcceleration); break;
                  case ControllerVecType_AngAccel: arrowVec = (Vector3f)(tracking.HandPoses[ctrl].AngularAcceleration); break;
                  default: OVR_FAIL(); break;
                  }

                  arrowVec *= vecScale;

                  for (int i = 0; i <= numVelocityCubes; ++i)
                  {
                    float lerpFac = (float)i / numVelocityCubes;
                    float cubeScale = (scaleEnd - scaleStart) * lerpFac + scaleStart;
                    Matrix4f cubeMat = CamFromWorld[camNum] * ctrlMat;
                    cubeMat = cubeMat * Matrix4f::Translation(arrowVec * lerpFac) * Matrix4f::Scaling(cubeScale);
                    cubeModel.Render(pRender, cubeMat);
                  }
                }
              };

              if (DrawControllerLinearVelocity)   drawControllerVec(SmallGreenCube, 1.000f, ControllerVecType_LinVeloc, false);
              if (DrawControllerAngularVelocity)  drawControllerVec(SmallBlueCube,  0.100f, ControllerVecType_AngVeloc, true);
              if (DrawControllerLinearAccel)      drawControllerVec(SmallGreenCube, 0.010f, ControllerVecType_LinAccel, false);
              if (DrawControllerAngularAccel)     drawControllerVec(SmallBlueCube,  0.001f, ControllerVecType_AngAccel, true);
            }
...
void OculusWorldDemoApp::RenderCamNearFarHandsView()
{
    ...
    EyeRenderPose[2] = ovrPosef(CamPose);  // external camera pose
...
void OculusWorldDemoApp::RenderCubeMapFromPosition(Texture* cubeColor, Texture* cubeDepth, int cubeMapSize, Vector3f translation)
{
  int numCubeFaces = 6;
  uint64_t format = GetRenderDeviceTextureFormatForEyeTextureFormat(EyeTextureFormat);

  AutoGpuProf prof(pRender, "Render_Cube");

  Matrix4f worldPose = Matrix4f::Translation(translation);
  ...
  for (int faceIndex = 0; faceIndex < numCubeFaces; ++faceIndex)
  {
    switch (faceIndex)
    {
    case 0: currCubemapCameraView = Matrix4f::RotationY(-MATH_FLOAT_PIOVER2) * worldPose; break; // Positive X
    case 1: currCubemapCameraView = Matrix4f::RotationY(MATH_FLOAT_PIOVER2) * worldPose; break; // Negative X
    case 2: currCubemapCameraView = Matrix4f::RotationZ(MATH_FLOAT_PI) * Matrix4f::RotationX(-MATH_FLOAT_PIOVER2) * worldPose; break; // Positive Y
    case 3: currCubemapCameraView = Matrix4f::RotationZ(MATH_FLOAT_PI) * Matrix4f::RotationX(MATH_FLOAT_PIOVER2) * worldPose; break; // Negative Y
    case 4: currCubemapCameraView = Matrix4f::RotationY(MATH_FLOAT_PI) * worldPose; break; // Positive Z
    case 5: currCubemapCameraView = worldPose; break; // Negative Z
    }
    ...
    pRender->SetRenderTarget(cubeColor, cubeDepth, NULL, faceIndex);
    ...
    // Give an optional matrix and boolean indicating not to render certain layers
    RenderEyeView(CamRenderPose_Left, ovrEye_Left, &currCubemapCameraView, true);

    if (cubeColor->GetFormat() & Texture_GenMipmaps)
    {
      cubeColor->GenerateMips();
    }
  }
  ...
  if (TextureOriginAtBottomLeft)
  {
    ...
    pRender->BltFlipCubemap(cubeColor, tempTex);
  }
}

void OculusWorldDemoApp::RenderCubemap()
{
    int cubeMapSize = LayerCubemapRenderResolution;
    Matrix4f currCubemapCameraView; // This will replace ViewFromWorld[eye] in RenderEyeView
    ...
    RenderCubeMapFromPosition(color, depth, cubeMapSize, transl);

    // Save cubemap texture to DDS file if Direct3D
    if (LayerCubemapSaveOnRender && !TextureOriginAtBottomLeft)
    {
        std::string error;
...
Recti OculusWorldDemoApp::RenderTextInfoHud(float textHeight)
{
    ...
    case Text_Info:
    {
        ...
        for (int camNum = 0; camNum < CamRenderPoseCount; camNum++ )
        {
            ...
            std::strncpy ( fovText, fovTextLine, sizeof (fovText) - 1 );
            fovText[sizeof(fovText) - 1] = '\0';
        ...
        snprintf (fovTextLine, sizeof(fovTextLine),
            ...
            sqrtPixels,
            sqrtPixels );
        std::strncat ( fovText, fovTextLine, sizeof (fovText) - strlen(fovText) - 1 );
...
void OculusWorldDemoApp::HmdColorDescChange(OptionVar*)
{
    ovrResult result = ovr_SetClientColorDesc(Session, &HmdColorDesc);
    OVR_UNUSED(result);
    OVR_ASSERT(result == ovrSuccess);
}
